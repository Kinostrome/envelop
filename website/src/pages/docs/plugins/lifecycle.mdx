...
<Callout type="info">
  All plugin lifecycle methods are executed in LIFO order (Last-in, First-out) to make timing more consistent.
</Callout>

### The Improved `context`

While in regular GraphQL executions, the `context` is built right before `execute` happens, in `envelop` we allow plugin developers to take part in that context from all phases.
...
You can find the [complete signature for plugins API here](https://github.com/n1ru4l/envelop/blob/d4c8f90f9fbccf3032b3b3b05f359b691076d25b/packages/types/src/index.ts#L55).
...
- `context` - the eventually built context, by all plugins.
- `extendContext` - extends the context object with additional fields.
