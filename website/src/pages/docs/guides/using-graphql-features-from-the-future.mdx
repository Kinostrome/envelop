...
The community expressed their need for a way of describing such polymorphic inputs and three RFCs popped up:
...
This means for the following operation:
...
Adding support to the existing envelop setup is straightforward:
...
As developers are using either the SDL first or code first approach envelop supports both ways.
...
For fragments a way of passing down variables in a capsulated way is impossible.
