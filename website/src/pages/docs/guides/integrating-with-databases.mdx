...
In this guide, we will cover a few pieces of advice to work with databases. The examples will use the Postgres node client but
the concepts apply to every database.
...
In this example, you can very quickly starve your API with a simple request :
...
Because we use directly the pool of connections, each resolver will request a client to do its request.
If you have, for example, 100 bills, this request can starve the connection pool for other users.
The server will be stuck and unresponsive to any incoming request while it is not fully resolved.
...
Now that we have one unique client during the execution phase, we can add some fancy features.
