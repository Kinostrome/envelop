...
Rolling out your authentication solution is a crucial task as any flaws can result in severe security issues.
...
In some cases using a third-party auth provider is not possible. But no worries, the generic auth plugin has you covered!
...
This plugin is perfect for use-cases where you want the whole schema to be introspectable, but restrict access to a certain part of the Graph only to specific users. E.g. in a paid subscription model, where API users should only have access to the data that is included within the plan.
...
For people migrating from apollo-server the [`useApolloServerErrors`](/plugins/use-apollo-server-errors) plugin provides full backward-compatibility to how apollo-server handles GraphQL error masking.
...
One of the main benefits of GraphQL is that data can be requested individually. However, this also introduces the possibility for attackers to send operations with deeply nested selection sets that could block other requests from being processed. Fortunately, infinite loops are not possible by design as a fragment cannot self-reference itself. Unfortunately, that still does not prevent possible attackers from sending selection sets that are hundreds of levels deep.
...
This can prevent malicious API users from executing GraphQL operations with deeply nested selection sets. You need to tweak the maximum depth an operation selection set is allowed to have based on your schema and needs, as it could vary between users.

### Rate Limiting

Rate-limiting is common practice with APIs, and with GraphQL it gets more complicated because of the flexibility of the graph and the ability to choose what fields to query.

The [`useRateLimiter`](/plugins/use-rate-limiter) limits access to resources, by a field level.
